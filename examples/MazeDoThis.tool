...
class MazeArray {
  ...
  def init(sze : Int) : MazeArray = {
    ...
    wallIDs = shuffleArray(wallIDs);

    i = 0;
    while(!allMerged()) {
      destroyIfNotConnected(wallIDs[i]);
      i = i + 1;
    }
  ...
  def setRepresentative(cid : Int, repr : Int) : Bool = {
    cells[cellRepresentative(cid)] = cellRepresentative(repr);
    return true;
  }
  ...
  def allMerged() : Bool = {
    ...
    i = 1;
    while(i < cellCount && cellRepresentative(i) == cellRepresentative(0)) {
      i = i + 1;
    }
  ...
  def cellOneOfWall(wid : Int) : Int = { return cellID(wallRow(wid), wallCol(wid)); }

  def cellTwoOfWall(wid : Int) : Int = { 
    var toRet : Int;

    if(wid < vertOffset) {
      toRet = cellID(wallRow(wid) + 1, wallCol(wid));
    } else {
      toRet = cellID(wallRow(wid), wallCol(wid) + 1);
    }
    return toRet;
  }
  ...
  def destroyIfNotConnected(wid : Int) : Bool = {
    ...
    c1 = cellOneOfWall(wid);
    c2 = cellTwoOfWall(wid);

    if(!(cellRepresentative(c1) == cellRepresentative(c2))) {
      walls[wid] = 0;
      setRepresentative(c1, c2);
    }
  ...
  def printMaze() : String = {
    ...
    while (i < size) {
      ...
      if(i == size - 1)
        str = str + l3Char;
      else if(0 < walls[wallID(false, 0, i)])
        str = str + t1Char;
      else
        str = str + horzChar;
      i = i + 1;
    ...
    while (i < size) {
      ...
      if(i < (size-1)) {
        ...
        while (j < size) {
          ...
          if(j < size - 1) {
            w1 = (walls[wallID(false, i, j)] == 1);       // up
            w2 = (walls[wallID(true, i, j + 1)] == 1);    // right
            w3 = (walls[wallID(false, i + 1, j)] == 1);   // down
            w4 = (walls[wallID(true, i, j)] == 1);        // left
            ...
            if(w1 && w2 && w3 && w4) 
              ...
              str = str + i2Char;
          } else if(walls[wallID(true, i, size - 1)] == 1)
            str = str + t2Char;
          else
            str = str + pipeChar;
    ...
    while (i < size) {
      ...
      if(i == size - 1)
        str = str + l4Char;
      else if(0 < walls[wallID(false, size-1, i)])
        str = str + t3Char;
      else
        str = str + horzChar;
      i = i + 1;
